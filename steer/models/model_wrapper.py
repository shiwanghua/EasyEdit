...
class BaseModelWrapper:
    ...
    def batch_get_logits(self, tokens, batch_size=4):
        # 计算总批次数
        num_batches = (len(tokens) + batch_size - 1) // batch_size
        print(f'len(tokens)={len(tokens)}, num_batches={num_batches}')
        # 存储每个批次的 logits
        all_logits = []
        # 按批次处理 tokens
        for i in range(num_batches):
            ...
            # 获取当前批次的 logits
            batch_logits = self.model(batch_tokens).logits
            # 将当前批次的 logits 添加到 all_logits 中
            all_logits.append(batch_logits)
        # 将所有批次的 logits 合并成一个 tensor
        all_logits = t.cat(all_logits, dim=0)
        return all_logits
